...
export const analyzeReportCard = async (base64Image: string, mimeType: string): Promise<ReportCardData> => {
  ...
  const prompt = `
    ...
    Tente também extrair nome da instituição certificadora o se visível.
    Se encontrar o nome da instituição certificadora (ex.: Secretaria de Educação, Secretaria Estadual, Prefeitura, INSTITUTO FEDERAL), inclua esse nome no campo "certifyingInstitution".
